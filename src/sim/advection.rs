...
pub fn advect_scalar(source: &ScalarField, velocity: &MacVelocity, dt: f32, destination: &mut ScalarField) {
    assert_eq!(source.grid(), velocity.u.grid(), "scalar and velocity grids must match");
    assert_eq!(source.grid(), destination.grid(), "source and destination scalar grids must match");
...
mod tests {
    ...
    use super::{advect_scalar, advect_velocity, sample_scalar, sample_velocity};
